...
def get_member_repo(request: Request) -> SyncInmemoryMemberRepo:
    return request.app.state.member_repo


def get_module_repo(request: Request) -> SyncInmemoryModuleRepo:
...
DepMemberRepo = Annotated[SyncInmemoryMemberRepo, Depends(get_member_repo)]
DepModuleRepo = Annotated[SyncInmemoryModuleRepo, Depends(get_module_repo)]
